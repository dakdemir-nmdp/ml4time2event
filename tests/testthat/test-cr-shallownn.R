...
test_that("ml4t2e_fit fits ShallowNN Competing Risk model", {
  set.seed(42)
  n_obs <- 100
  ...
  # Fit
  fit <- ml4t2e_fit(keep_data = TRUE, task, models = "cr_shallownn")

  expect_s3_class(fit, "t2e_fit")
  expect_true("cr_shallownn" %in% fit$model_names)

  wrapper <- fit$models$cr_shallownn
  expect_true(inherits(wrapper, "ShallowNNCompetingRiskModel"))
  expect_type(wrapper$model, "list") # List of models per cause
  expect_true(length(wrapper$model) >= 2)
})

test_that("ShallowNN Competing Risk predictions format", {
  set.seed(99)
  n_obs <- 60
  train_data <- data.frame(
    time = rexp(n_obs),
