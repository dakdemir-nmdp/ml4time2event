...
# --- Tests for ApplyDictToData ---

# Setup data and dictionary for ApplyDictToData tests
apply_test_data <- data.frame(
  ID = c("1", "2", "3", "4"),
  Age = c("30", "45", "55", "60"),
  Sex = c("M", "F", "M", " F "), # Extra space
  Score = c("10.5", "20.1", "NA", "30.9"), # NA as string
  Status = c("Active", "Inactive", "Active", "Unknown"), # Unknown is NA
  JoinDate = c("2023-01-15", "2023/02/20", "2024-03-10", "2024/04/05"),
  ExtraCol = 1:4, # Not in dict
  stringsAsFactors = FALSE
)

apply_test_dict <- data.frame(
  Variable = c("ID", "Age", "Sex", "Sex", "Score", "Status", "Status", "Status", "JoinDate"),
  Type = c("Character", "Continuous", "Categorical", "Categorical", "Numeric", "Categorical", "Categorical", "Categorical", "Date"),
  Value = c(NA, NA, "M", "F", NA, "Active", "Inactive", "Unknown", NA),
  Label = c("Identifier", "Age (Years)", "Male", "Female", "Risk Score", "Active Status", "Inactive Status", "Unknown Status", "Date Joined"),
  NACats = c("VALUE", "VALUE", "VALUE", "VALUE", "VALUE", "VALUE", "VALUE", "NAVAL", "VALUE"), # Mark 'Unknown' as NA
  Description = c("Unique ID", "Patient Age", "Sex Male", "Sex Female", "Calculated Score", "Patient Status Active", "Patient Status Inactive", "Patient Status Unknown", "Enrollment Date"),
  stringsAsFactors = FALSE
)
# Add a dict entry with NA value marked as NA
apply_test_dict_na_val <- rbind(apply_test_dict,
                                data.frame(Variable="Score", Type="Numeric", Value="NA", Label="NA Score", NACats="NAVAL", Description="NA Score", stringsAsFactors = FALSE))


test_that("ApplyDictToData converts types correctly", {
  data_applied <- ApplyDictToData(apply_test_dict, apply_test_data)

  expect_type(data_applied$ID, "character")
  expect_type(data_applied$Age, "double") # Continuous/Numeric -> double
  expect_s3_class(data_applied$Sex, "factor")
  expect_type(data_applied$Score, "double")
  expect_s3_class(data_applied$Status, "factor")
  expect_s3_class(data_applied$JoinDate, "Date")
  expect_type(data_applied$ExtraCol, "integer") # Untouched
})

test_that("ApplyDictToData applies factor levels and labels", {
  data_applied <- ApplyDictToData(apply_test_dict, apply_test_data)

  # Check Sex factor
  expect_equal(levels(data_applied$Sex), c("M", "F")) # Levels from dict Value
  # Check values (note: " F " in data should match "F" level after trimws)
  expect_equal(as.character(data_applied$Sex), c("M", "F", "M", "F"))

  # Check Status factor (Unknown is NA, should not be a level)
  expect_equal(levels(data_applied$Status), c("Active", "Inactive"))
  expect_equal(as.character(data_applied$Status), c("Active", "Inactive", "Active", NA))
})

test_that("ApplyDictToData handles NA values based on NACats", {
  data_applied <- ApplyDictToData(apply_test_dict, apply_test_data) # Uses NACats="NAVAL" for Status="Unknown"
  expect_true(is.na(data_applied$Status[4]))
  expect_false(any(is.na(data_applied$Status[1:3])))

  # Test NA handling for Score where "NA" string is present
  expect_true(is.na(data_applied$Score[3])) # Should be NA due to coercion
  # Test explicit NA value mapping
  data_applied_na_val <- ApplyDictToData(apply_test_dict_na_val, apply_test_data)
  expect_true(is.na(data_applied_na_val$Score[3])) # Should be NA due to NACats="NAVAL"
})

test_that("ApplyDictToData handles date conversion", {
  data_applied <- ApplyDictToData(apply_test_dict, apply_test_data)
  expect_equal(data_applied$JoinDate, as.Date(c("2023-01-15", "2023-02-20", "2024-03-10", "2024-04-05")))
})

test_that("ApplyDictToData handles missing/mismatched columns", {
  dict_missing_var <- apply_test_dict[!apply_test_dict$Variable == "Age", ]
  data_applied_missing <- ApplyDictToData(dict_missing_var, apply_test_data)
  expect_type(data_applied_missing$Age, "character") # Age should remain character

  dict_extra_var <- rbind(apply_test_dict, data.frame(Variable="NonExistent", Type="Numeric", Value=NA, Label="X", NACats="VALUE", Description="Y", stringsAsFactors = FALSE))
  # expect_warning(ApplyDictToData(dict_extra_var, apply_test_data), "not found in data") # Check for warning if implemented
  data_applied_extra <- ApplyDictToData(dict_extra_var, apply_test_data)
  expect_equal(ncol(data_applied_extra), ncol(apply_test_data)) # No new column added
})

test_that("ApplyDictToData handles case-insensitive matching", {
  dict_lower <- apply_test_dict
  dict_lower$Variable <- tolower(dict_lower$Variable)
  data_applied_lower <- ApplyDictToData(dict_lower, apply_test_data)
  # Check if types were applied despite case difference
  expect_type(data_applied_lower$Age, "double")
  expect_s3_class(data_applied_lower$Sex, "factor")
})
