...
test_that("ml4t2e_fit fits GAM Competing Risk model", {
  ...
  fit <- ml4t2e_fit(keep_data = TRUE, task, models = "cr_gam")

  expect_s3_class(fit, "t2e_fit")
  expect_true("cr_gam" %in% fit$model_names)

  wrapper <- fit$models$cr_gam
  expect_true(inherits(wrapper, "GamCompetingRiskModel"))
  expect_type(wrapper$model, "list")
  expect_true(all(c("1", "2") %in% names(wrapper$model)))
})

test_that("GAM Competing Risk predictions format", {
  skip_if_not_installed("mgcv")
