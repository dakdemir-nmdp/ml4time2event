...
test_that("ml4t2e_fit fits GLMNet Survival model", {
    ...
    # Fit
    fit <- ml4t2e_fit(keep_data = TRUE, task, models = "glmnet")

    expect_s3_class(fit, "t2e_fit")
    expect_true("glmnet" %in% fit$model_names)

    wrapper <- fit$models$glmnet
    expect_true(inherits(wrapper, "GlmnetSurvivalModel"))
    expect_s3_class(wrapper$model, "cv.glmnet")
})

test_that("GLMNet Survival predictions format", {
    skip_if_not_installed("glmnet")
