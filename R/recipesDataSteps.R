...
t2emodel_data_recipe_init<-function(timevar, eventvar, expvar,idvars,  traindata){
  ...
  out<-recipes::recipe(frml,
                       data = traindata[,c(timevar,eventvar,expvar,idvars)])
  out$vars<-c(timevar,eventvar,expvar, idvars)
  return(out)
}
...
prep_data_recipe<-function(model_recipe,training){
  vars<-model_recipe$vars
  out<-recipes::prep(model_recipe, training = training[, model_recipe$vars])
  out$vars<-vars
  return(out)
}
###############
...
out<-recipes::bake(model_recipe, data[, model_recipe$vars])
return(out)
}