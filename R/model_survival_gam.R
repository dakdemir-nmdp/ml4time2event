#' Generalized additive survival model (R6 implementation)
#'
#' Uses the existing `SurvModel_GAM()` helper to fit a smooth Cox model while
#' exposing the unified `TimeToEventModel` interface.
#'
#' @keywords internal
#' @noRd
GamSurvivalModel <- R6::R6Class(
  ...
  public = list(
    ...
    fit = function(task, time_grid, ...) {
      ...
      self$model <- do.call(SurvModel_GAM, args)
    ...
    predict_survival = function(newdata, times, set = "test", ...) {
      ...
      model_name <- self$spec$engine %||% "gam"
    ...
    model_info = function() {
      ...
      info$label <- "Generalised additive model"
      info
    },
