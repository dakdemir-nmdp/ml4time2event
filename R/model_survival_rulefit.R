#' RuleFit survival model (R6 implementation)
#'
#' Bridges the legacy `SurvModel_rulefit()` helper into the new object-oriented
#' workflow so the rule-based ensemble can be trained and scored via
#' `ml4t2e_fit()`.
#'
#' @keywords internal
#' @noRd
RulefitSurvivalModel <- R6::R6Class(
  ...
  public = list(
    ...
    fit = function(task, time_grid, ...) {
      ...
      self$model <- do.call(SurvModel_rulefit, args)
    ...
    predict_survival = function(newdata, times, set = "test", ...) {
      ...
      model_name <- self$spec$engine %||% "rulefit"
      ...
      if (any(complete_idx)) {
        ...
        pred <- Predict_SurvModel_rulefit(
          modelout = self$model,
