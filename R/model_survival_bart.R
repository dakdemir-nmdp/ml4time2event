#' Bayesian additive regression trees for survival (R6 implementation)
#'
#' Wraps `BART::surv.bart()` via the existing `SurvModel_BART` helper so the
#' engine participates in the new object-oriented workflow.
#'
#' @keywords internal
#' @noRd
BartSurvivalModel <- R6::R6Class(
  ...
  public = list(
    ...
    predict_survival = function(newdata, times, set = "test", ...) {
      ...
      model_name <- self$spec$engine %||% "bart"
