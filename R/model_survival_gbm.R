#' Gradient boosted survival trees (R6 implementation)
#'
#' Wraps the existing `SurvModel_gbm()` helper so the GBM engine participates in
#' the new registry-driven workflow.
#'
#' @keywords internal
#' @noRd
GbmSurvivalModel <- R6::R6Class(
  ...
  public = list(
    ...
    fit = function(task, time_grid, ...) {
      ...
      self$model <- do.call(SurvModel_gbm, args)
    ...
    predict_survival = function(newdata, times, set = "test", ...) {
      ...
      model_name <- self$spec$engine %||% "gbm"
