...
Predict_SurvModel_xgboost <- function(modelout, newdata, newtimes = NULL) {
  ...
  has_missing <- any(is.na(X_new))
  n_complete_cases <- sum(complete.cases(X_new))
  n_expected_obs <- nrow(X_new)

  if (has_missing) {
    warning(sprintf(
      "Missing values detected in %d observation(s). ",
      n_expected_obs - n_complete_cases),
      "xgboost prediction may fail or return NAs for incomplete cases."
    )
  }

  # Ensure X_new has the same columns as training (in case of missing factor levels)
  missing_cols <- setdiff(modelout$model$feature_names, colnames(X_new))
  for(col in missing_cols){
    X_new[[col]] <- 0 # Add missing columns with 0
  }
  # Ensure correct column order
  X_new <- X_new[, modelout$model$feature_names, drop = FALSE]
  ...
  preds <- tryCatch(
    predict.xgb.Booster.surv(actual_model, dtest, type = "surv", times=times),
    error = function(e) {
      # If prediction fails due to missing values, create NA matrix
      if (has_missing) {
        warning("XGBoost prediction failed due to missing values, returning NAs")
        matrix(NA, nrow = length(times), ncol = n_expected_obs)
      } else {
