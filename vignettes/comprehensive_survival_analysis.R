...
# Using Lung Cancer Dataset (lung_dataset.csv)
...
data_path <- system.file("extdata", "lung_dataset.csv", package = "ml4time2event")
if (data_path == "") {
  # Fallback for development - direct path
  data_path <- "inst/extdata/lung_dataset.csv"
}

# Use readData function from the package
lung_data <- readData(data_path)

# Display basic information about the dataset
cat("Dataset dimensions:", nrow(lung_data), "rows,", ncol(lung_data), "columns\n")
cat("Column names:", paste(names(lung_data), collapse = ", "), "\n")
cat("First few rows:\n")
print(head(lung_data))

# Check variable types and missing data
cat("\nVariable summary:\n")
print(summary(lung_data))

# --- 3. Data Preprocessing ---
cat("\n=== STEP 2: DATA PREPROCESSING ===\n")

# Clean and prepare the data
# Remove institution column as it's not a clinical predictor
lung_data <- lung_data[, !names(lung_data) %in% c("inst")]

# Convert categorical variables to factors
lung_data$sex <- factor(lung_data$sex, levels = c(1, 2), labels = c("Male", "Female"))
lung_data$ph.ecog <- factor(lung_data$ph.ecog)

# Use package's comprehensive imputation function to handle all missing values
# Exclude time and status variables from imputation as they are outcomes
lung_data <- ImputeMissinRecordsData(
  lung_data, 
  dontuse = c("time", "status"),
  num.trees = 100  # Use reasonable number of trees for imputation
)

# Create additional derived variables
lung_data$age_group <- cut(lung_data$age, breaks = c(0, 50, 65, 100),
                          labels = c("Young", "Middle", "Elderly"))
lung_data$performance_good <- ifelse(lung_data$ph.karno >= 80, 1, 0)

# Check the event status coding
cat("Event status distribution:\n")
print(table(lung_data$status))
cat("Time range (days):", range(lung_data$time), "\n")
cat("Event rate:", round(mean(lung_data$status == 1) * 100, 1), "%\n")

# Variable profiling using package function
candidate_expvars <- c("age", "sex", "ph.ecog", "ph.karno", "pat.karno",
...
tryCatch({
  rf_model <- SurvModel_RF(
    ...
    timevar = timevar,
    eventvar = eventvar,
    ntree = 500,                              # More trees for better performance
    samplesize = min(100, nrow(train_data)),   # Larger sample size for adequate dataset
    splitrule = "logrank",                    # Better split rule for survival
    nodesize_try = c(5, 10, 15)              # Reasonable node sizes
  )
  models[["RF"]] <- rf_model
...
tryCatch({
  xgb_model <- SurvModel_xgboost(
    ...
    nrounds = 200         # More rounds with conservative learning rate
  )
...
tryCatch({
  gbm_model <- SurvModel_gbm(
    ...
    eventvar = eventvar,
    ntree = 100,
    bag.fraction = 0.8  # Standard bag fraction for adequate dataset size
  )
...
tryCatch({
  rulefit_model <- SurvModel_rulefit(
    ...
    timevar = timevar,
    eventvar = eventvar,
    ntree = 500,                              # More trees for rule diversity
    nsample = min(nrow(train_data), 100),     # Better sampling for adequate dataset
    alpha = 0.1,                              # Moderate L1 penalty
    maxit = 5000                              # More iterations for convergence
  )
...
# 5.9 DeepSurv (Neural Network) Survival Model
...
tryCatch({
  deepsurv_model <- SurvModel_DeepSurv(
    ...
    timevar = timevar,
    eventvar = eventvar,
    size = 8,                                 # Larger network for adequate dataset
    decay = 0.01,                             # Lower regularization
...
# Identify complete cases in test_data for consistent predictions
complete_test_data <- test_data[complete.cases(test_data[, expvars]), ]
cat("Using", nrow(complete_test_data), "complete cases out of", nrow(test_data), "test observations\n")

for (model_name in model_names) {
  if (model_name %in% names(models) && model_name %in% names(predict_functions)) {
    cat("Generating", model_name, "predictions...\n")
    tryCatch({
      pred <- predict_functions[[model_name]](models[[model_name]], complete_test_data)
...
# Define evaluation time points (in days for lung cancer)
eval_times <- quantile(actual_times[actual_events_binary == 1], c(0.25, 0.5, 0.75))
cat("Evaluation time points (days):", round(eval_times, 1), "\n")
...
cox_brier <- cox_brier_obj$AppErr$model[1]  # Extract the Brier score value
...
for (model_name in names(predictions)) {
  tryCatch({
    ...
    brier_val <- brier_obj$AppErr$model[1]  # Extract the Brier score value
...
# We'll use rownames of complete_test_data as patient IDs (if not present, set them)
if (is.null(rownames(complete_test_data)) || any(rownames(complete_test_data) == "")) {
  rownames(complete_test_data) <- as.character(seq_len(nrow(complete_test_data)))
}
patient_ids <- rownames(complete_test_data)
...
for (j in seq_len(ncol(cox_pred$Probs))) {
  tryCatch({
    ...
    cat("Warning: Interpolation failed for Cox model patient", j, ":", e$message, "\n")
    cox_interp[, j] <- NA
  })
}
# Set consistent column names using patient_ids
colnames(cox_interp) <- patient_ids
  ...
  if (model_name != "Cox" && model_name %in% names(predictions)) {
    ...
    for (j in seq_len(ncol(pred$Probs))) {
      tryCatch({
        ...
        cat("Warning: Interpolation failed for", model_name, "patient", j, ":", e$message, "\n")
        model_interp[, j] <- NA
      })
    }
    # Set consistent column names using patient_ids
    colnames(model_interp) <- patient_ids
...
ensemble_brier_obj <- tryCatch({
  ...
  cat("Ensemble Brier score calculation failed:", e$message, "\n")
  NULL
})

# Extract the Brier score value from the pec object
ensemble_brier <- if (!is.null(ensemble_brier_obj) && !is.null(ensemble_brier_obj$AppErr)) {
  ensemble_brier_obj$AppErr$model[1]
} else {
  NA
}
...
# Add ensemble Brier score to the pred_errors list
brier_scores[["Ensemble"]] <- ensemble_brier

# Define prediction errors as Brier scores at median time (updated with ensemble)
pred_errors <- brier_scores
...
cat("90th percentile follow-up time (days):", round(max_time, 1), "\n")
...
for (j in seq_len(ncol(cox_pred$Probs))) {
  tryCatch({
    ...
    cat("Warning: ETL interpolation failed for Cox model patient", j, ":", e$message, "\n")
    cox_interp[, j] <- NA
  })
}
...
for (model_name in names(predictions)) {
  tryCatch({
    ...
    for (j in seq_len(ncol(pred$Probs))) {
      tryCatch({
        if (all(is.na(pred$Probs[, j])) || length(pred$Probs[, j]) == 0) {
          model_interp[, j] <- NA
        } else {
          model_interp[, j] <- approx(pred$Times, pred$Probs[, j],
                                     xout = common_times, method = "constant",
        ...
        cat("Warning: ETL interpolation failed for", model_name, "patient", j, ":", e$message, "\n")
        model_interp[, j] <- NA
      })
    }
    model_interp_list[[model_name]] <- model_interp
  }, error = function(e) {
    cat("Interpolation failed for", model_name, ":", e$message, "\n")
    # Create a matrix of NAs as fallback
    model_interp_list[[model_name]] <- matrix(NA, nrow = length(common_times), 
...
for (model_name in names(model_interp_list)) {
  tryCatch({
    ...
    etl_results[[model_name]] <- rep(NA, ncol(interp_probs))  # Placeholder
  })
}
...
tryCatch({
  ...
  ensemble_etl <- rep(NA, length(patient_ids))
  etl_results[["Ensemble"]] <- ensemble_etl
})
...
if (requireNamespace("gridExtra", quietly = TRUE)) {
  ...
  # Save to file
  ggsave("lung_survival_analysis.pdf", combined_plot, 
         width = 20, height = 20, units = "in")
  cat("Plots saved to lung_survival_analysis.pdf\n")
}
...
saveRDS(cox_model, "cox_model_lung.rds")

# Save all trained models
saved_models <- c("cox_model_lung.rds")
for (model_name in names(models)) {
  filename <- paste0(tolower(model_name), "_model_lung.rds")
...
saveRDS(ensemble_object, "ensemble_model_lung.rds")
saved_models <- c(saved_models, "ensemble_model_lung.rds")
...
loaded_cox <- readRDS("cox_model_lung.rds")
loaded_rf <- readRDS("rf_model_lung.rds")
loaded_ensemble <- readRDS("ensemble_model_lung.rds")
...
cat("Dataset: Lung Cancer (NCCTG Trial, n =", nrow(lung_data), ")\n")
...
cat("Dataset: Lung Cancer (NCCTG Trial)\n")
cat("Training set: 160 subjects\n")
cat("Test set:", nrow(complete_test_data), "subjects (complete cases)\n")
