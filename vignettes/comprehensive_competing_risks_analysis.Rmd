...
We examine the event counts for relapse (1), treatment-related mortality (2),
and censoring (0).
...
# 5. Cumulative Incidence by Disease Phase

`cuminc()` from **cmprsk** highlights how relapse and treatment-related
mortality differ across disease phase at transplant.
...
# 7. Competing-Risks Model Training

We train an ensemble of competing-risks learners with `RunCRModels()`.
...
Evaluate performance using time-dependent concordance, Brier score, and expected 
time lost. The updated API provides consistent metric calculation for competing 
risks models using the standardized `eval_times` parameter.
...
# 10. Expected Time Lost Analysis
...
- **Enhanced Cox model**: `CRModel_Cox()` now supports `ntimes`, `alpha` (elastic net regularization), and `nfolds` (cross-validation folds) for competing risks analysis with optional penalized regression.

- **XGBoost improvements**: `CRModel_xgboost()` now includes `ntimes` (prediction grid) and `verbose` (progress reporting) parameters for better control and monitoring.

- **Unified metric API**: `BrierScore()` and `BrierScoreCR()` now use the consistent `eval_times` parameter (standardized across survival and competing risks) for seamless model evaluation.

- **Automatic time attribute propagation**: Predictions now include a `Times` attribute for automatic time grid tracking through the analysis pipeline, including CIF matrices.

## Workflow Summary

- Prepared dataset `bmt_competing_risks_prepared` (via `get_bmt_competing_risks_data()`) removes manual preprocessing from the vignette code.
- `RunCRModels()` and `PredictCRModels()` orchestrate multiple competing-risks learners and ensembles with consistent time grid management.
- Evaluation utilities include time-dependent concordance, Brier score, and expected time lost tailored to competing risks with unified API.
- Visual diagnostics highlight patient-level CIFs as well as cohort-level phase comparisons.

## Migration Notes

- If you have existing code using `time` parameter in `BrierScoreCR()`, the package maintains backward compatibility by accepting both `time` (scalar) and `eval_times` (vector).
- Individual model predictions are now consistently structured with `Times` attribute for easy access to prediction time points, including CIF matrices.
